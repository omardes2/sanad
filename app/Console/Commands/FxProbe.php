 ...
 *  set-reporting-currency <code> <expected_current>  → ok:<code> | stale | rejected:<rule>
 */
class FxProbe extends Command
{
    ...
    public function handle(FxPairBook $pairs, FxRateBook $rates, ReportingConversionService $conversions, ReportingCurrencyService $currency): int
    {
        /** @var list<string> $a */
        ...
        try {
            $line = match ((string) $this->argument('op')) {
                ...
                'set-reporting-currency' => 'ok:'.$currency->change($a[0], $a[0], $a[1]),
