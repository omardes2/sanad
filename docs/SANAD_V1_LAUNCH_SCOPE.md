...
| **Follow-Up Until Done** | نطاق V1 يشترط متابعة تعمل بلا إزعاج. **منفَّذة** الآن، فالبند صار **يُبلِّغ** بدل أن يُعلن غيابًا: المتابعة **حلقة مفتوحة** في `follow_ups`، وكل سؤال **صفّ `reminders` عاديّ** بمطالبته وميزانيته ورسالته الصادرة — و**سؤال واحد معلَّق كحدّ أقصى** في كل وقت، فالسلّم لا يُنشأ مسبقًا. السلطة `UNIQUE (follow_up_id, ask_index)` مع `status`/`version` تحت قفل. **الإنشاء يحتاج سلطتين**: طلب صريح من المشترك (`ExplicitFollowUpIntent`) **و**وقت حدَّده هو؛ وبلا وقت **يسأل سَنَد ولا يفترض** — لا «بكرا» ولا «بعد ٢٤ ساعة» ولا أي افتراض. **وميزانية الأسئلة حقيقةُ تسليم**: السؤال الذي بلغ «أُرسل» (`status = sent`) وحده؛ فالمطالبة والمحاولة المُؤذَن بها والمرفوض قبل الإرسال وموتُ العامل قبل الشبكة **لا تستهلك شيئًا** — و`attempts` سقفُ إنفاق فيزيائي لا إثباتُ سؤال. والمنتهي بلا «أُرسل» (ومنه النتيجة غير المعروفة) **يَحجِز** الحلقة بلا استهلاك وبلا سؤال بديل. والإغلاق بجواب المشترك **المرتبط بالسؤال** أو بإنجاز المهمة المرتبطة أو بإلغائه أو بنفاد الميزانية — و**الصمت لا يُغلق شيئًا**، والكلام غير المفهوم ليس «لا» ولا يُنتج سؤالًا فوريًا. والحدود من الإعداد (٥ متابعات مفتوحة · ٣ أسئلة · ٢٤ ساعة بين سؤالين · نافذة جواب ٧٢ ساعة) (ADR-0052). و`PlanFeature::FollowUp` **مستقلّة** عن `Reminders`. | تشغيلي: تفعيل المتابعة + قالب المتابعة |
| **`WHATSAPP_FOLLOW_UP_TEMPLATE_READY` — قالب واتساب معتمَد للمتابعة** | سؤال المتابعة («دفعت الفاتورة؟») **ليس تذكيرًا**، واعتماد Meta يكون **لكل قالب على حدة** — فقالب التذكيرات غير معتمَد دلاليًّا لسؤال، واستعماله يعني كلامًا غير معتمَد أمام المشترك. ولذلك هوية إعداد **منفصلة** وبند **منفصل**: بلا قالب متابعة، السؤال المستحقّ خارج نافذة الخدمة **لا يُنشأ أصلًا** — تُحجَز المتابعة «موقوفة تشغيليًا» (`blocked`) **بلا استهلاك ميزانية** و**بلا تسليم فاشل متكرِّر**، والاستعادة أمر تشغيلي صريح وغير متكرِّر الأثر (`sanad:follow-ups:unblock`). **ولا تُقدَّم المتابعة لمشتركي الإنتاج قبل اعتماد القالب وضبطه فعليًّا.** | خارجي: اعتماد قالب Meta |
