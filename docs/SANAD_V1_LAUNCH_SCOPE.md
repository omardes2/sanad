...
| **Voice Notes / التفريغ الصوتي** | نطاق V1 يشترط أن يرسل المستخدم رسالة صوتية ويحصل على فهم ورد. لا يوجد أي منفِّذ تفريغ في `app/`: الموجود مفردات فقط (`MessageType::Audio` · `AiOperation::Transcription` · `UsageDimension::VoiceMessage`). حالة البند `NOT IMPLEMENTED`، **ولا يُخمَّن مزوّد**. | مرحلة الصوت |
...
| **فرض الحصص والفوترة (`BILLING_ENFORCE`)** | مفتاح تشغيلي افتراضه معطّل: بيتا قد تعمل بلا فرض عن قصد. يُعرَض في شاشة الجاهزية لأن أثره على التكلفة كبير، ولا يحجب الإطلاق. |
