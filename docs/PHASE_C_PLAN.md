...
| 5 | Cache invalidation | الكتابة + Audit في transaction واحدة؛ `CatalogCache::flush()` **بعد** commit ناجح؛ rollback ⇒ لا تغيير ولا Audit؛ فشل الـflush بعد commit لا يفشل الكتابة (تحذير `sanad.ai.catalog_cache_unavailable` + TTL 60 ثانية). |
