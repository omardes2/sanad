...
|---|---|---|
| Credential lifecycle | `pending / active / revoked`؛ أي مفتاح جديد أو Rotation يبدأ `pending` ولا يؤثر على الـruntime؛ المسار: create pending → Test Connection (auth ناجح لنفس الصف) → activate → revoke previous active | `CredentialManager` (الكاتب الوحيد): `create()` يختم بالخزنة ويُدرج pending؛ `activate(credential, expectedCurrentActiveId)` داخل transaction واحدة مع `FOR UPDATE` على صف `ai_providers`: يجب أن يكون الصف pending، **ويُعاد قراءة الفعّال الحالي تحت القفل ويجب أن يطابق ما رآه المستخدم** (`expected_active_id` من الصفحة) وإلا يُرفض كـstale conflict ويبقى الصف pending بلا revoke (لا last-writer-wins)، ويُفتح بالخزنة الحالية، **ويوجد فحص مصادقة ناجح لنفس `credential_id`** (`provider_health_checks` kind=auth, status=ok خلال `ai.health.verification_window_minutes` = 30 (eligibility للتفعيل فقط؛ الاحتفاظ بالسجل 90 يومًا مستقل)) — بلا فحص أو فحص فاشل/قديم/لصف آخر ⇒ مرفوض؛ ثم يُلغى الفعّال السابق ويُفعَّل الجديد معًا — فشل أي خطوة (بما فيها الـAudit) يترك القديم Active. مسار قسري منفصل `activateUnverified()` لـsuper_admin فقط، بتأكيد مكتوب `UNVERIFIED`، **فقط** لمزوّد لا يعلن adapter‑ه probe غير مفوتر (يُرفض لـOpenAI/Groq)، وAudit مستقل `ai.credentials.activated_unverified` بـ`forced=true, verified=false`. فهرس partial unique `provider_credentials_one_active_per_provider`. اختبار PostgreSQL حقيقي بعمليات نظام متزامنة (`sanad:ai-credential-probe {credential} {expected}`): 10 صفوف pending موثَّقة تنطلق كلها من نفس الفعّال الحالي ⇒ واحد ينجح، القديم revoked، 9 تبقى pending (لا revoke للخاسر)؛ 10 تفعيلات للصف نفسه ⇒ واحد ينجح و9 تُرفض بنظافة. |
