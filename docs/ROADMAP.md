...
## ✅ Sprint 3 — Subscriptions, Plans & Usage Engine

- [x] باقات يديرها المشغّل بالكامل (JSON limits/features — لا أسعار/حدود في الكود).
- [x] اشتراك مستقل لكل مشترك (channel-agnostic) + onboarding تجربة تلقائي بلا تكرار.
- [x] محرّك استخدام مركزي: check قبل الذكاء + charge ذرّي idempotent آمن ضد التزامن.
- [x] إنفاذ الحدود اليومية/الشهرية/غير المحدود/المعطّل عبر مُزخرِف حول منسّق الذكاء.
- [x] أساس تكلفة (ledger `usage_events` بأسعار قابلة للضبط، افتراضي صفر) + جاهزية أرصدة موزونة.
- [x] لوحة تحكم: الباقات، المشتركون، تفاصيل المشترك (استخدام/حدود/متبقٍّ + إجراءات يدوية).
- [x] اختبارات شاملة (باقات، تجربة، لا تكرار، حدود، عزل، idempotency، إنفاذ قبل الذكاء).

**غير مشمول:** الدفع/CyberSource، أتمتة واتساب التجارية، المكالمات، أرصدة مفروضة، ذاكرة طويلة المدى.

## 🔜 Sprint التالي — (مقترح)

- [ ] تحضير بوّابة الدفع (عقد PaymentGateway + placeholder) — بلا شحن حيّ.
