...
| `/dashboard/finance/close` (E4، `finance.view` للقراءة؛ الإقفال/إعادة الفتح `finance.close_period` = super_admin فقط) | Preflight لشهر UTC (المقاييس السبعة أو `NOT AVAILABLE`، الشروط المانعة/المعلوماتية، hash)، **Close** (كتابة `CLOSE YYYY-MM`)، **Reopen** (كتابة `REOPEN YYYY-MM` + سبب + دليل)، سجل الإقفالات والمراجعات مع `DRIFT SINCE CLOSE`. finance يرى الصفحة للعرض فقط. |
