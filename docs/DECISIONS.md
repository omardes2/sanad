...
## ADR-0035 — عدّادات استخدام ذرّية آمنة ضد التزامن
- **القرار:** جدول `usage_counters` (فريد لكل subscriber/dimension/period/period_key)
  يُقفَل `FOR UPDATE` داخل معاملة للفحص+الزيادة معًا؛ وسجلّ `usage_events` بمفتاح
  `idempotency_key` فريد يمنع الشحن المزدوج (تكرار ويبهوك/إعادة محاولة).
- **السبب:** منع تجاوز الحدّ الصارم تحت التزامن (لا read-then-increment)، وعدم احتساب مكرر.
