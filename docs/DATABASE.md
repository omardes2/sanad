...
- **`fingerprint` بصمة مفتاحية (HMAC-SHA256) للنص المطبَّع، لا SHA256 عاريًا**، بمفتاح مستقل ثانٍ (`MEMORY_FINGERPRINT_KEY`). السبب: بصمة غير مفتاحية تصبح **أوراكل تأكيد**؛ من يملك نسخة من قاعدة البيانات يخمّن جملة ويحسب بصمتها ليعرف إن كان المشترك يتذكّرها، **بلا مفتاح التشفير أصلًا**. الذاكرات قصيرة ونمطية فالتخمين رخيص، والمفتاح يزيل ذلك.
