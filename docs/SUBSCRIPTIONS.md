...
- `charge()` بعد النجاح: **upsert ذرّي شرطي** على `usage_counters`
  ...
  العدّاد بعد** (سباق أول رسالتين): القيد الفريد يُسلسِل الإنشاء، والشرط يمنع التجاوز. **idempotent**
  عبر `usage_events.idempotency_key` الفريد (مفتاح = معرّف الرسالة الواردة) ⇒ تكرار الويبهوك
  أو إعادة محاولة المهمة لا يُحاسب مرتين. (لماذا ليس `FOR UPDATE`؟ لأنه لا يقفل صفًّا غير موجود؛
  انظر ADR-0035.) اختبار تزامن حقيقي على PostgreSQL يثبت الثبات (`PostgresConcurrencyTest`).
