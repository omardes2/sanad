...
| جدول | المحتوى |
|---|---|
| `subscription_events` (append-only، بلا `updated_at`) | `subscription_id`, `subscriber_id` (مرجعان تاريخيان بلا FK)، `event_type`, `from_status?`, `to_status`, `from_plan_id?`, `to_plan_id?`, `effective_at` (UTC)، `source` (baseline/admin/onboarding/system/gateway)، `actor_ref`, `reason?`, `correlation_id?`, `metadata?`, `baseline_key?` (فريد `sub:<id>`)، `created_at`. فهارس (subscription, effective_at)، (subscriber, effective_at)، type، source. |
...
- `PostgresHistoryConcurrencyTest`: 6 تغييرات مالية متزامنة على باقة واحدة ⇒ نسخة مفتوحة واحدة، فترات متجاورة مرتّبة بلا تداخل؛ 5 تشغيلات baseline متزامنة ⇒ baseline واحد لكل اشتراك، نسخة واحدة لكل باقة، audit واحد؛ 6 انتقالات متزامنة ⇒ 6 أحداث بسلسلة متسقة و6 سجلات تدقيق.
