...
it('adds and removes cost_invoices_period_start_id_idx reversibly without touching the table, its rows or the other E2 indexes', function () {
    ...
    expect($files)->toHaveCount(63)
        ->and(basename($files[count($files) - 5]))->toBe('2026_09_06_001303_add_period_start_index_to_cost_invoices_table.php')
    ...
    Artisan::call('migrate:rollback', ['--step' => 5, '--force' => true]); // the F1 migration, the two F2 ones and the reminder-delivery one sit on top of this one
