...
it('adds and removes cost_adjustments.idempotency_key and its unique index reversibly without touching the table or its rows', function () {
    ...
    expect($files)->toHaveCount(62)
        ->and(basename($files[count($files) - 5]))->toBe('2026_09_06_001302_add_idempotency_key_to_cost_adjustments_table.php')
    ...
    Artisan::call('migrate:rollback', ['--step' => 5, '--force' => true]); // the period index migration, the F1 one and the two F2 ones sit on top of this one
