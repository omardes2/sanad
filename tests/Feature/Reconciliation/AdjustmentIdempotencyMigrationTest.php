...
it('adds and removes cost_adjustments.idempotency_key and its unique index reversibly without touching the table or its rows', function () {
    ...
    expect($files)->toHaveCount(67)
        ->and(basename($files[count($files) - 10]))->toBe('2026_09_06_001302_add_idempotency_key_to_cost_adjustments_table.php')
    ...
    Artisan::call('migrate:rollback', ['--step' => 10, '--force' => true]); // the period index migration, the F1 one, the two F2 ones, the reminder-delivery one, the memory one, the voice-transcription one, the recurring-reminders one and the follow-up one sit on top of this one
