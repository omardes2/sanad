...
/** E5.2b — the ONE migration of the phase: nullable idempotency_key + unique index on cost_adjustments, reversible, no backfill. */
it('adds and removes cost_adjustments.idempotency_key and its unique index reversibly without touching the table or its rows', function () {
    ...
    expect($files)->toHaveCount(58)
        ->and(basename(end($files)))->toBe('2026_09_06_001302_add_idempotency_key_to_cost_adjustments_table.php')
