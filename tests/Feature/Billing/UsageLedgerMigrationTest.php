...
it('back-fills derived ledger columns for rows that pre-date the ledger and stays reversible', function () {
    // Undo the F1 migration (tool_consents), the two E5.2b migrations (period_start index on cost_invoices, idempotency_key on cost_adjustments),
    ...
    // +1: the reminder-delivery migration now sits on top of all of them, and
    // +1 again for the voice-transcription columns. The boundary migration
    // itself is unchanged; only the distance to it grew.
    Artisan::call('migrate:rollback', ['--step' => 45, '--force' => true]);
