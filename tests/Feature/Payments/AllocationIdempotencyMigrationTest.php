...
it('adds and removes the allocation idempotency columns and unique indexes reversibly on both tables, without touching existing rows', function () {
    ...
    expect($files)->toHaveCount(59) // E5.2a (allocations) + E5.2b (cost_adjustments key, cost_invoices period index) — counted from the actual files
        ->and(basename($files[count($files) - 3]))->toBe('2026_09_06_001301_add_idempotency_key_to_allocation_tables.php')
    ...
    Artisan::call('migrate:rollback', ['--step' => 3, '--force' => true]); // the two E5.2b migrations sit on top of this one
