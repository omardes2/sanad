...
it('adds and removes the allocation idempotency columns and unique indexes reversibly on both tables, without touching existing rows', function () {
    ...
    expect($files)->toHaveCount(67) // E5.2a (allocations) + E5.2b (cost_adjustments key, cost_invoices period index) + F1 (tool_consents) + F2 (two invocation tables) + reminder delivery + memory — counted from the actual files
        ->and(basename($files[count($files) - 11]))->toBe('2026_09_06_001301_add_idempotency_key_to_allocation_tables.php')
    ...
    Artisan::call('migrate:rollback', ['--step' => 11, '--force' => true]); // the two E5.2b migrations, the F1 one, the two F2 ones, the reminder-delivery one, the memory one, the voice-transcription one, the recurring-reminders one and the follow-up one sit on top of this one
