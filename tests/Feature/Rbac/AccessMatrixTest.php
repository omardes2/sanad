...
it('grants finance.reconcile (Phase E2) to super_admin and finance only', function () {
    expect(userWithRole(Role::SuperAdmin)->can('finance.reconcile'))->toBeTrue()
        ->and(userWithRole(Role::Finance)->can('finance.reconcile'))->toBeTrue()
        ->and(userWithRole(Role::Operations)->can('finance.reconcile'))->toBeFalse()
        ->and(userWithRole(Role::Support)->can('finance.reconcile'))->toBeFalse();
...
it('Phase E5.2a payment routes reuse finance.payments.manage: finance and super_admin 200, operations/support/legacy admin/no-role 403, guests redirected', function () {
    ...
    $fx = closableMonth();
    $urls = [
        route('dashboard.finance.payments.show', $fx['usd']->id),
        route('dashboard.finance.refunds', ['from' => '2026-08-01', 'to' => '2026-08-31']),
        route('dashboard.finance.refunds.show', $fx['refund']->id),
    ];
